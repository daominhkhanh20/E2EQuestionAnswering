...
def calculate_input_training_for_qav3(examples, tokenizer, max_length):
    # print(len(examples))
    ...
    for i, offsets in enumerate(offset_mapping):
        ...
        if len(examples[ANSWER][sample_index]) == 0:
            tokenized_examples["start_positions"].append(cls_index)
            tokenized_examples["end_positions"].append(cls_index)
        else:
            # Start/end character index of the answer in the text.
            start_char = examples[ANSWER_START][sample_index]
            end_char = start_char + len(examples[ANSWER])
            ...
            if not (offsets[token_start_index][0] <= start_char and offsets[token_end_index][
                ...
                while token_start_index < len(offsets) and offsets[token_start_index][
                    0] <= start_char:
                    token_start_index += 1
